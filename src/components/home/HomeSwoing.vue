<!--<template>
  <div class="swoing">
    <div class="swoingshow">
      <ul>
        <transition-group tag="ul" name="image">
          <li v-for="(img, index) in imgArrary" :key="index" v-show="index === mark" @touchstart.capture="touchStart" @touchend.capture="touchEnd">
            <a href="#">
              <img :src="img">
            </a>
          </li>
    ...
    <div class="bar">
      <span v-for="(item,index) in imgArrary" :key="index" :class="{'active':index === mark}"></span>
    </div>
  </div>
</template>
      ...
      methods:{
        ...
        touchEnd (e) {
          ...
          if (this.startX - this.endX > 30) {
            console.log('左滑')
            /*if (this.mark >= this.imgArrary.length - 1) {
              // 不做操作
            } else {*/
              this.mark++
            if(this.mark === 3){
              ...
              /*document.getElementById('activityUl').style.left = -this.mark * this.liWidth + 'px'
            }*/
          }
          // 右滑
          if (this.startX - this.endX < -30) {
...
<template>
  <div>
    <wc-swiper :duration="1500" :interval="2500" class="swiper">
      <wc-slide v-for="(img, index) in imgArrary" :key="index">
        <img :src="img">
      </wc-slide>
    </wc-swiper>
  </div>
